...
```{r setlocale, echo=FALSE, message=FALSE, warning=FALSE, error=FALSE, results='hide'}
...
To create this report the data was downloaded on: `r filedate`
...
The mean and the median are different in this case (`r as.integer(mean)` and `r as.integer(median)` steps respectively).
...
The interval that contains the maximun number of steps is `r maxInterval` with `r round(max)` steps on average across all days.

## Imputing missing values
The original data frame has `r sum(is.na(activitydata$steps))` missing values. The presence of missing days may introduce bias into some calculations or summaries of the data.
...
The mean and median now have the same value (`r as.integer(mean)` steps). The result indicates that ignoring NA values introduced bias to the initial analysis.
